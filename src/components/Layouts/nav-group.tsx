    ...
    <SidebarGroup>
      ...
      <SidebarMenu>
        {items.map((item) => {
          ...
          if (!item.items)
            return (
              <SidebarMenuLink key={key} item={item} location={location} />
            );
          ...
          return (
            <SidebarMenuCollapsible key={key} item={item} location={location} />
          );
        })}
      </SidebarMenu>
    </SidebarGroup>
...
function checkIsActive(pathname: string, item: any, mainNav = false) {
  return (
    ...
    pathname.startsWith(item.url) || // Match nested routes
    !!item?.items?.some((i: any) => pathname.startsWith(i.url)) || // If child nav is active
    (mainNav &&
      pathname.split('/')[1] !== '' &&
