...
export default function AdminstudentSettingsPage() {
  ...
  const fetchstudentList = async () => {
    try {
      const response = await api.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/student/manual/getList`,
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditing) {
      await api.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/student/edit`,
        newstudent
      );
      setstudents(
      ...
      const response = await api.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/student/register/manual`,
        newstudent
      );
  ...
  const handleDelete = async () => {
    if (deleteConfirmation.studentId) {
      try {
        await api.delete(
          `${import.meta.env.VITE_API_BASE_URL}/api/student/delete`,
          {
